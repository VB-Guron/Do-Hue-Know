...
def calc_Green():
    data_folder = Path(__file__).parent / "girl_voicelines" / "02. Amplified" / "Green.wav"


    y1, sr1 = librosa.load(data_folder)  # Color library - do not move
    y2, sr2 = librosa.load(Path(__file__).parent / "VoiceInput.wav", sr=44100)  # Put the unity voice-line file here

    # load data
    rate, data =  wavfile.read(Path(__file__).parent / "VoiceInput.wav")
