...
def calc_Orange():
    data_folder =Path(__file__).parent.parent.parent / "girl_voicelines" / "02. Amplified" / "Orange.wav"
    ...
    sf.write(Path(__file__).parent.parent.parent /'FilteredAudio.wav', new_y, sr2)

    y3, sr3 = librosa.load(Path(__file__).parent.parent.parent /'FilteredAudio.wav', sr=None)

    yt, index = librosa.effects.trim(y5, top_db=20, frame_length=400, hop_length=124)
    sf.write(Path(__file__).parent.parent.parent /'TrimmedFinalAudio.wav', yt, sr3)
